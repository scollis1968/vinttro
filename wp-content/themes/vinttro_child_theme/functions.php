 ...
 */
/**
 * Dynamic VINTTRO Pinned Header Profile Dropdown Array - Adaptive Integration Patches
 */
function vinttro_render_account_avatar_menu() {
    ...
    $settings_page_url = home_url( '/visp/dashboard/' ); 
    $account_logout_url = wp_logout_url( home_url() );   
    ?>
    
    <div id="vinttro-avatar-template-source" style="display:none;">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Find the native right menu item bar element container
        const targetContainer = document.querySelector('.mobmenur-container');
        const templateSource = document.getElementById('vinttro-avatar-template-source');
        
        if (!targetContainer || !templateSource) return;

        // Extract the avatar block node tree from footer storage and prepend it directly into the menu structure bar
        const avatarNode = templateSource.querySelector('.vinttro-global-account-node');
        targetContainer.prepend(avatarNode);

        // Bind layout events to our moved nodes
        const trigger = avatarNode.querySelector('.vinttro-avatar-trigger-btn');
        const dropdown = avatarNode.querySelector('.vinttro-account-dropdown');
