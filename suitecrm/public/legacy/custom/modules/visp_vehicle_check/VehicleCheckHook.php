...
class VehicleCheckHook 
{
    ...
    public function beforeSaveMethod($bean, $event, $arguments) 
    {
        ...
        // Try to get the vehicle ID from the relationship framework
        $vehicleId = '';
        ...
        // FALLBACK: If this is a brand-new record from a subpanel, the relationship isn't saved yet.
        // We grab the ID straight from the browser submission request.
        if (empty($vehicleId)) {
        ...
        // Helper closures to cleanly handle custom fields that might or might not have '_c'
        ...
        // 2. Set data on parent vehicle (auto-handling potential _c suffixes)
        ...
        if (!empty($vDateLastService)) {
            ...
            if (!empty($bean->date_of_check) && !empty($bean->mileage) && !empty($vMilesLastService)) {
                ...
                if ($milesDriven > 0 && $daysElapsed > 0 && (float)$vServiceIntervalMiles > 0) {
                    ...
                    $GLOBALS['log']->fatal("VCHook DBG: Run-rate skipped. Miles Driven: $milesDriven, Days Elapsed: $daysElapsed, Target Interval: $vServiceIntervalMiles");
                }
            }
            ...
            $GLOBALS['log']->fatal("VCHook DBG: Vehicle date_last_service is empty; skipping timeline math.");
        }
