...
function vinttro_get_cover_admin_panel($user_id) {
    ...
    // 💡 FIX 1: Provide feedback if no RFQs exist instead of returning an empty string
            ...
            <tbody>
                <?php foreach ($rfqs as $rfq) : 
                    ...
                    $irfq_config = [
                        'new' => [
                            'color' => '#210fe8e0',
                            'desc'  => 'New no response from insurer',
                            'count' => 0
                        ],
                        'rejected' => [
                            'color' => '#89170a',
                    ...
                    // 💡 FIX 2: Safely parse vehicle title fallback
                    $car_title = esc_attr(trim(($rfq['make'] ?? '') . ' ' . ($rfq['model'] ?? '')));
                ?>
                    <!-- 💡 FIX 3: Corrected malformed style attribute to class -->
                    <tr class="rfq-row main-row <?php echo $has_insurer_rfqs ? 'has-issues' : 'no-issues'; ?>">
                        <td class="rfq-cell">
                            <?php 
                            $crm_id = $rfq['id'] ?? ''; 
                            ...
                            if (!empty($crm_id)) : ?>
                                <a href="<?php echo esc_url(rtrim($crm_base_url, '/')) . '/#/visp_cover_rfq/record/' . esc_attr($crm_id); ?>" 
                                   target="_blank" 
                                   title="View in CRM: <?php echo $car_title; ?>"
                                   style="text-decoration: none; color: inherit;">
                                    <span class="vehicle-reg"><?php echo $reg_text; ?></span>
                                </a>
                            <?php else : ?>
                                <span class="vehicle-reg" title="<?php echo $car_title; ?>">
                                    <?php echo $reg_text; ?>
                                </span>
                            <?php endif; ?>
                                ...
                                <div class="issues-expanded-box">
                                    <ul class="issue-detailed-list" style="list-style: none; padding-left: 0; margin-top: 4px;">
                                        <?php foreach ($insurer_rfqs as $insurer_rfq) : 
                                            ...
                                            // 💡 FIX 4: Corrected $sev_config to $irfq_config
